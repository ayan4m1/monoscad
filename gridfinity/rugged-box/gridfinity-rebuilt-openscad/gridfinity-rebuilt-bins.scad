...
if (top_lip_chamfer > 0) {
    translate([0, -(gridy * l_grid) / 2, height(gridz, gridz_define, style_lip, enable_zsnap) + (top_lip_chamfer / 1.5)]) chamfer_edge_mask(l = (gridx * l_grid) + 10, chamfer = top_lip_chamfer, orient = RIGHT);
}
}
gridfinityBase(gridx, gridy, l_grid, div_base_x, div_base_y, style_hole, only_corners=only_corners);
}

module cylinderSlot(diameter, height, rounding = 3) {
    cylinder(d = diameter, h = height, center = true);
    rotate([0, 0, cyl_rotation]) translate([0, 0, diameter / 3]) scale(cyl_scoop_dim_scale) sphere(d = diameter);
    rotate([0, 0, -cyl_rotation]) translate([0, 0, diameter / 3]) scale(cyl_scoop_dim_scale) sphere(d = diameter);
    translate([0, 0, height / 2]) rounding_hole_mask(d = diameter, rounding = rounding);
}

translate([120, 0, 0]) color("teal") {
difference() {
gridfinityInit(gridx, gridy, height(gridz, gridz_define, style_lip, enable_zsnap), height_internal, sl=style_lip) {
    translate([0, ((gridy * l_grid - big_cyl_diam) / 2) - cyl_inset * 2, height(gridz, gridz_define, style_lip, enable_zsnap) - 10]) cylinderSlot(big_cyl_diam + cyl_padding, big_cyl_height);
    //translate([-((gridx * l_grid - big_cyl_diam) / 2) +cyl_inset, ((gridy * l_grid - big_cyl_diam) / 2) - cyl_inset, height(gridz, gridz_define, style_lip, enable_zsnap) - 10]) cylinderSlot(big_cyl_diam, big_cyl_height);
    translate([0, -((gridy * l_grid - big_cyl_diam) / 2) + cyl_inset * 2, height(gridz, gridz_define, style_lip, enable_zsnap) - 10]) cylinderSlot(big_cyl_diam + cyl_padding, big_cyl_height);
    translate([-((gridx * l_grid - small_cyl_diam) / 2) + cyl_inset * 2, 0, height(gridz, gridz_define, style_lip, enable_zsnap) - 7.5]) cylinderSlot(small_cyl_diam + cyl_padding, small_cyl_height);
    translate([((gridx * l_grid - small_cyl_diam) / 2) - cyl_inset * 2, 0, height(gridz, gridz_define, style_lip, enable_zsnap) - 7.5]) cylinderSlot(small_cyl_diam + cyl_padding, small_cyl_height);
    //translate([((gridx * l_grid - big_cyl_diam) / 2) - cyl_inset, -((gridy * l_grid - big_cyl_diam) / 2) + cyl_inset, height(gridz, gridz_define, style_lip, enable_zsnap) - 10]) cylinderSlot(big_cyl_diam, big_cyl_height);
}
if (top_lip_chamfer > 0) {
...
/*
translate([120, 0, 0]) color("blue") {
difference() {
gridfinityInit(gridx, gridy, height(gridz, gridz_define, style_lip, enable_zsnap), height_internal, sl=style_lip) {
    translate([((gridx * l_grid - big_cyl_diam) / 2) - cyl_inset, ((gridy * l_grid - big_cyl_diam) / 2) - cyl_inset * 8, height(gridz, gridz_define, style_lip, enable_zsnap) - 10]) cylinderSlot(big_cyl_diam, big_cyl_height);
    translate([-((gridx * l_grid - big_cyl_diam) / 2) +cyl_inset, ((gridy * l_grid - big_cyl_diam) / 2) - cyl_inset, height(gridz, gridz_define, style_lip, enable_zsnap) - 10]) cylinderSlot(big_cyl_diam, big_cyl_height);
    translate([-((gridx * l_grid - big_cyl_diam) / 2) + cyl_inset, -((gridy * l_grid - big_cyl_diam) / 2) + cyl_inset * 8, height(gridz, gridz_define, style_lip, enable_zsnap) - 10]) cylinderSlot(big_cyl_diam, big_cyl_height);
    translate([((gridx * l_grid - big_cyl_diam) / 2) - cyl_inset, -((gridy * l_grid - big_cyl_diam) / 2) + cyl_inset, height(gridz, gridz_define, style_lip, enable_zsnap) - 10]) cylinderSlot(big_cyl_diam, big_cyl_height);
}
